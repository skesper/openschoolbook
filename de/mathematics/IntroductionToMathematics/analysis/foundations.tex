...
\begin{definition}
Eine Folge heißt \textsl{monoton steigend}\index{monoton steigend}, wenn für alle $i\in \mathbb{N}$ gilt $a_i\le a_{i+1}$. Sie heißt \textsl{streng monoton steigend}, wenn $a_i < a_{i+1}$ gilt. Äquivalent heißt sie \textsl{monoton fallend}\index{monoton fallend}, wenn $a_i\ge a_{i+1}$ gilt, bzw. \textsl{streng monoton fallend}, wenn $a_i > a_{i+1}$ gilt.
\end{definition}
...
\begin{definition}
Eine reelle Folge ($a_i \in \mathbb{R}$) heißt \textsl{beschränkt}, wenn es ein $S\in \mathbb{R}$ gibt, sodass für alle $i\in \mathbb{N}$, $a_i \le S$ gilt. $S$ ist eine \textsl{obere Schranke}\index{Schranke, obere} der Folge. Gibt es eine kleinste obere Schranke $S'$, so wird diese \textsl{Supremum}\index{Supremum} genannt. 

Gibt es ein $I\in \mathbb{R}$ mit $a_i \ge I$, so wird $I$ \textsl{untere Schranke}\index{Schranke, untere} genannt. Die größte untere Schranke wird \textsl{Infimum}\index{Infimum} genannt.
\end{definition}

\begin{definition}
Eine Folge, deren Elemente alle identisch sind, wird \textsl{konstante Folge}\index{konstante Folge} genannt. Eine Folge, deren Elemente abwechselnd positiv und negativ sind, wird \textsl{alternierend}\index{alternierend} genannt. Eine Folge deren Elemente sich immer weiter der 0 annähern, wird \textsl{Null-Folge}\index{Null-Folge} genannt.
\end{definition}

\subsection{Konvergenz}

\begin{definition}\label{def:lim}
Es sei $(a_i)$ eine Folge in den reellen Zahlen und $a\in \mathbb{R}$ eine Zahl. Weiterhin sei $\epsilon>0$ eine beliebige reelle Zahl. Wenn es für jeden solchen Wert $\epsilon$ einen Folgenindex $n_\epsilon \in \mathbb{N}$ gibt, sodass $\vert a_i -a\vert <\epsilon$ für alle $i\ge n_\epsilon$ gilt, so heißt die Folge \textsl{kovergent}\index{konvergent}. Ist die Folge konvergent gegen $a$, so nennt man $a$ den \textsl{Grenzwert}\index{Grenzwert} der Folge $a_i$.
\end{definition}
...
\begin{definition}
Es sei $(a_i)$ eine Folge. Es sei $\epsilon \ll 1$. Wenn es zu jedem $\epsilon$ ein $N_\epsilon \in \mathbb{N}$ gibt, sodass für $n,m > N_\epsilon $ gilt $\vert a_n -a_m \vert <\epsilon$, so ist auch diese Folge konvergent. Bemerkenswert dabei ist, dass wir nur die Abstände der Folgenelemente dafür betrachten und keinen Grenzwert. Eine solche Folge wird \textsl{Cauchy-Folge}\footnote{benannt nach dem französischen Mathematiker \textbf{Augustin-Louis Cauchy}, *21. August 1789, \ding{61}23. Mai 1857 in Sceaux}\index{Cauchy-Folge} genannt.
\end{definition}
...
\begin{definition}
Sei $(a_i)$ eine Folge in $A\subseteq \mathbb{R}$. Wenn die Folge gegen einen Wert $a\in A$ konvergiert, so heißt $a$ \textsl{Grenzwert}, oder auch \textsl{Limes}\footnote{Limes lat. "`Grenzweg"'}\index{Limes}\index{lim} der Folge $(a_i)$. Dass $(a_i)$ den Grenzwert $a$ besitzt, wird wie folgt ausgedrückt:
\[
...
existiert, so heißt $A$ \textsl{vollständig}\index{vollständig}. 

Die Vollständigkeit der reellen Zahlen in der Form, dass alle Cauchy-Folgen konvergieren und ihren Grenzwert annehmen, ist eine Konsequenz aus dem sogenannten \textsl{Vollständigkeitsaxiom}\index{Vollständigkeitsaxiom}. Die Vollständigkeit folgert also nicht aus der Konvergenz der Cauchy-Folgen, sondern genau umgekehrt folgert die Konvergenz der Cauchy-Folgen aus der Vollständigkeit. 
\end{definition}


Der Begriff \textsl{Axiom}\index{Axiom} bezeichnet einen Satz oder eine Voraussetzung innerhalb einer Theorie, der/die beweislos als richtig angesehen wird. Er leitet sich vom griechischen "`\begin{greek}'axi'wmata\end{greek}"' -- "`axi\'omata"' her: Würdiger, anerkannter Satz.
...
s_n = \sum_{i=1}^{n} a_i
\]
die $n$-te \textsl{Partialsumme}\index{Partialsumme}. Die Partialsummen bilden wiederum eine Folge $(s_i)$. Falls der Grenzwert 
\[
R(a) = \lim_{n\rightarrow \infty} s_n = \lim_{n\rightarrow \infty} \sum_{i=1}^{n} a_i
\]
existiert, so heißt $R(a)$ eine \textsl{Reihe}\index{Reihe}.
\end{definition}
...
\begin{definition}
$f$ ist genau dann in $a$ \textsl{stetig}, wenn es zu jedem $\epsilon>0$ ein $\delta_\epsilon > 0$ gibt, so dass für alle $b\in A$ mit $\vert b-a\vert < \delta_\epsilon $ immer $\vert f(b)-f(a)\vert < \epsilon$ ist. Diese Definition wird meist $\epsilon$-$\delta$-Stetigkeit genannt. \index{$\delta$-$\epsilon$-stetig}
\end{definition}
