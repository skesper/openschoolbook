...
\begin{definition}
Sei $V$ ein K-Vektorraum mit einem Skalarprodukt $\langle .,.\rangle$. Die Linearform 
\[
\Vert u \Vert = \sqrt{\langle u,u \rangle}
\]
wird \textsl{Euklidische Norm} oder vom Skalarprodukt \textsl{induzierte Norm}. Sie ist hier separat definiert, da sie in diesem Kapitel eine zentrale Rolle spielt. 
\end{definition}

\begin{definition}
Eine Norm \textsl{Norm} ist eine lineare Abbildung $\Vert . \Vert : V \longrightarrow \mathbb{R}_+$, vom Vektorraum in die positiven reellen Zahlen. Dabei gelten folgende Eigenschaften:
\begin{description}
\item[(1)] Aus $\Vert v\Vert = 0$ folgt $v=0$
\item[(2)] $\Vert a\cdot v\Vert = \vert a\vert \cdot \Vert v\Vert$
\item[(3)] $\Vert u+v\Vert \le \Vert u\Vert + \Vert v\Vert $
\end{description}
Mit $\vert a\vert$ dem Absolutbetrag des Skalars $a$.
\end{definition}

Dass die Euklidische Norm die Eigenschaften (1)-(3) erfüllt, soll in den Aufgaben nachgerechnet werden.

Normen stellen ein wichtiges Werkzeug dar, denn mit ihnen können Längen von Vektoren gemessen werden. 
...
\begin{definition}
Ein \textsl{Skalarprodukt-Raum} ist ein K-Vektorraum $(V,+,\cdot)$ zusammen mit einem Skalarprodukt $\langle .,. \rangle$.
\end{definition}

Ein Beispiel für einen Skalarprodukt-Raum ist der $\mathbb{R}^n$, zusammen mit dem Skalarprodukt
\begin{equation*}
\langle u,v \rangle = \sum_{i=1}^{n} u_i \cdot v_i
\end{equation*}

TODO identisch mit Euklidischem Raum?
