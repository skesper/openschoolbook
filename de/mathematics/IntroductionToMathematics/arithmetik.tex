...
\begin{definition}
Als \textbf{Vorzeichen} wird entweder das $+$ oder das $-$ bezeichnet, das vor eine Zahl steht. Es bestimmt, ob die Zahl positiv ($+$) oder negativ ($-$) ist.
\end{definition}
...
\begin{definition}
Dies bezeichnen wir als \textbf{Division mit Rest}. 
\end{definition}
In unserem Beispiel würden man sagen:
\[ {11\over 3} = 3 \textsl{ Rest } 2 \]

Aber ist das wirklich einfacher? Sicher nicht. Das Ergebnis $11/3$ ist, vom mathematischen Standpunkt aus betrachtet, ein vollkommen korrektes Ergebnis und benötigt keine andere Darstellung. Daher tendiert man dazu, solche Ergebnisse in der Teilerform zu behalten und nicht weiter umzuformen. 
\begin{definition}
Eine Teilerform nennt man einen \textbf{Bruch}, sowie das Rechnen mit Brüchen \textbf{Bruchrechnung}.
\end{definition}

\subsection{Bruchrechnung}

\begin{definition}
Ein Bruch besteht aus drei Teilen. Oben steht eine Zahl, die als \textbf{Zähler} bezeichnet wird, unten eine Zahl, die \textbf{Nenner} genannt wird und dazwischen ist der \textbf{Bruchstrich}:
...
als \textbf{Rationale Zahlen} und stellen sie mit dem Symbol $\mathbb{Q}$ dar. 
\end{definition}
Also ist 
...
Rationale Zahlen besitzen zu ihrer Darstellung als Bruch auch eine \textbf{Dezimalbruchentwicklung}. Zur Erläuterung betrachten wir zunächst die Zahlen zwischen 0 und 1. Dies sind die Zahlen 
\[ 1 \over n\]
...
positive, negative, rationale und irrationale Exponenten.
